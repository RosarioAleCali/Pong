...
int main()
{
	std::cout << "Pong Instructions:" << std::endl;
	std::cout << "1) Press the space bar to start the game." << std::endl;
	std::cout << "2) Press the space bar to continue the game each time after a player scores." << std::endl;
	std::cout << "3) Press the close button to stop the game." << std::endl;
	...
	Paddle paddle = Paddle(100, 10, 20, (HEIGHT - 100) / 2, 1);
	Paddle paddle2 = Paddle(100, 10, WIDTH - 20, (HEIGHT - 100) / 2, 2);
	Ball puck = Ball(5, WIDTH / 2 + 5, (HEIGHT - 5) / 2 + 5);
...
void reset(Ball& puck, sf::Text& text) {
	puck.ball.setPosition(WIDTH / 2 + 5, (HEIGHT - 5) / 2 + 5);
	puck.velocity.x = 0;
	puck.velocity.y = 0;
